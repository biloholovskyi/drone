...
class Quadron_Wp_Bootstrap_Navwalker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        $item_output .= '<a'. $attributes .'><span class="chevron">';
        $item_output .= $args->link_before . apply_filters('the_title', $item->title, $item->ID) . $args->link_after;

        $item_output .= (($depth == 0 || 1) && $args->has_children) ? '</span></a>' : '</span></a>';
    ...
    public static function fallback($args)
    {
        if (current_user_can('edit_theme_options')) {
            echo '<li class="mt-35"><a href="' . admin_url('nav-menus.php') . '">' . esc_html__('Add a menu', 'quadron') . '</a></li>';
        }
    }
}