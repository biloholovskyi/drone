...
function quadron_posts_next_pag_attrs()
{
    return 'class="nt-pagination-link -next"';
}
add_filter('next_posts_link_attributes', 'quadron_posts_next_pag_attrs');
...
function quadron_posts_prev_pag_attrs()
{
    return 'class="nt-pagination-link -previous"';
}
add_filter('previous_posts_link_attributes', 'quadron_posts_prev_pag_attrs');
...
if (! function_exists('quadron_single_navigation')) {
    function quadron_single_navigation()
    {
        if ('0' != quadron_settings('single_navigation_visibility')) {
	...
	<nav class="nt-single-navigation -style-centered">
    	<ul class="nt-single-navigation-inner">
        	...
        	<li class="nt-single-navigation-item -prev">
	           <?php previous_post_link('%link', _x('PREVIOUS POST', 'Previous post link', 'quadron')); ?>
        	</li>

        	<li class="nt-single-navigation-item -next">
    	       <?php next_post_link('%link', _x('NEXT POST', 'Next post link', 'quadron')); ?>
        	</li>
...
if (! function_exists('quadron_single_navigation_two')) {
    function quadron_single_navigation_two()
    {
        if ('1' == quadron_settings('single_navigation_visibility','0')) {
...
function quadron_index_loop_pagination()
{
    if ('0' != quadron_settings('pagination_visibility', '1')) {
        $pag_class= array();
        $pag_class[] = ('yes' == quadron_settings('pag_group')) ? '-group' : '';
        $pag_class[] = '-style-'.quadron_settings('pag_type', 'outline');
        $pag_class[] = '-size-'.quadron_settings('pag_size', 'medium');
        $pag_class[] = '-corner-'.quadron_settings('pag_corner', 'circle');
        $pag_class[] = '-align-'.quadron_settings('pag_align', 'center');
    ...
    <ul class='nt-pagination-inner'>" . "\n";
        ...
        if (get_previous_posts_link()) {
            echo '<li class="nt-pagination-item">' . wp_kses($prev, quadron_allowed_html()) . '</li>';
        }
        ...
        if (get_next_posts_link()) {
            echo '<li class="nt-pagination-item">' . wp_kses($next, quadron_allowed_html()) . '</li>';
        }
...
function quadron_current_link_pages($link)
{
    if (ctype_digit($link)) {
...
add_filter('wp_link_pages_link', 'quadron_current_link_pages');
...
if (! function_exists('quadron_wp_link_pages')) {
    function quadron_wp_link_pages()
    {

        // pagination for page links
        wp_link_pages(array(

            'before' => '<div class="page-links"><span class="page-links-title">' . esc_html__('Pages', 'quadron') . '</span>',
            'after' => '</div>',
            'link_before' => '',
            'link_after' => '',
            'next_or_number' => 'number',
            'separator' => ' ',
            'nextpagelink' => esc_html__('Next page', 'quadron'),
            'previouspagelink' => esc_html__('Previous page', 'quadron'),
            'pagelink' => '%',
            'echo' => 1
