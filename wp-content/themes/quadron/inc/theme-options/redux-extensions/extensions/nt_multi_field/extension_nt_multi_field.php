        ...
        private function is_minimum_version () {
            ...
            if ($this->min_redux_version != '') {
                if (version_compare($redux_ver, $this->min_redux_version) < 0) {
                    $msg = '<strong>' . esc_html__( 'The', 'quadron') . ' ' .  $this->ext_name . ' ' .  esc_html__('extension requires', 'quadron') . ' Redux Framework ' . esc_html__('version', 'quadron') . ' ' . $this->min_redux_version . ' ' .  esc_html__('or higher.','quadron' ) . '</strong>&nbsp;&nbsp;' . esc_html__( 'You are currently running', 'quadron') . ' Redux Framework ' . esc_html__('version','quadron' ) . ' ' . $redux_ver . '.<br/><br/>' . esc_html__('This field will not render in your option panel, and features of this extension will not be available until the latest version of','quadron' ) . ' Redux Framework ' . esc_html__('has been installed.','quadron' );
                    ...
                    if (method_exists('Redux_Admin_Notices', 'set_notice')) {
                        ...
                        echo     '<p>';
                        echo quadron_sanitize_data($msg);
                        echo     '</p>';
                        echo '</div>';
                    }
